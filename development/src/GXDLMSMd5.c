...
static void CGXDLMSMD5_Encode(unsigned char * output, unsigned int *input, unsigned int len)
{
    unsigned int pos = 0;
...
static void CGXDLMSMD5_Transform(unsigned char* block, unsigned int *state)
{
...
static int CGXDLMSMD5_Update(unsigned char* data, unsigned int len, unsigned char* buffer, unsigned int* count, unsigned int* state)
{
    unsigned int i;
    // Number of bytes.
    ...
    if (len >= firstpart)
    {
        ...
        CGXDLMSMD5_Transform(buffer, state);

        // Transform block
        for (i = firstpart; i + 64 <= len; i += 64)
        {
            CGXDLMSMD5_Transform(data + i, state);
        }
        index = 0;
    }
    else
...
int CGXDLMSMD5_Encrypt(CGXByteBuffer* data, CGXByteBuffer* digest)
{
    ...
    CGXByteBuffer_Capacity_2(digest, 16);

    CGXDLMSMD5_Update(data->m_Data, data->m_Size, buffer, (unsigned int*)&count, (unsigned int*)&state);
    ...
    CGXDLMSMD5_Encode(bits, (unsigned int*)&count, 2);

    // Pad out to 56 mod 64.
    unsigned int index = count[0] / 8 % 64;
    unsigned int padLen = (index < 56) ? (56 - index) : (120 - index);
    CGXDLMSMD5_Update(padding, padLen, buffer, (unsigned int*)&count, (unsigned int*)&state);

    // Append length (before padding)
    CGXDLMSMD5_Update(bits, 8, buffer, (unsigned int*)&count, (unsigned int*)&state);

    // Store state in digest
    CGXDLMSMD5_Encode(digest->m_Data, state, 4);
    CGXByteBuffer_SetSize(digest, 16);
    return 0;
}
